...
if($_SERVER['REQUEST_METHOD']=='POST'){
  ...
  if (!in_array($dayName, $allowedDays, true)) {
    $_SESSION['error'] = "The selected day (" . $dayName . ") is not one of the scheduled course days (" . $course['days'] . ")";
    header("Location:../pages/teacher_attendance.php?course_id=$course_id&teacher_id=$teacherId");
    exit;
    }
    // تحقق: التاريخ ضمن مدة الكورس
  elseif($selectedDate < $course['start_date'] || $selectedDate > ($course['postpone_date'] ?: $course['end_date'])) {
    $_SESSION['error'] = "The selected date is outside the course period.";
    header("Location:../pages/teacher_attendance.php?course_id=$course_id&teacher_id=$teacherId");
    exit;
  } 
  else{
